...
possible to use conditions for debugging and then switch them off
for distribution.
...
class conditions(object):
    ...
    def __call__(self, function):
        # Combine recursive wrappers
        # (@precondition + postcondition == @conditions)
        pres = set((self.__precondition,))
        posts = set((self.__postcondition,))

        # Unwrap function, collect distince pre-/post conditions
        ...
        # Add a wrapper for each pair (not that 'conditions' may be empty)
